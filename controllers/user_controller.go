...
func LoginUser(w http.ResponseWriter, r *http.Request) {
	...
	WHERE email = ? 
	AND password = ?`, email, password).Scan(&user.IdUser, &user.Name_user, &user.Age, &user.Address, &user.Email, &user.Password, &user.User_type)
	...
	if errQuery != nil {
		...
		generateToken(w, user.IdUser, user.Name_user, user.User_type)
		response.Status = 200
		response.Message = "The request was successful"
		response.Data = user
	}
